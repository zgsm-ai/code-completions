...
def extract_block_prefix_suffix(sitter, choices_text, prefix, suffix):
    ...
    code = sitter.get_text(block_node)
...
def extract_accurate_block_prefix_suffix(sitter, prefix, suffix):
    ...
    cur_block_code = sitter.get_text(sitter.find_second_level_node_by_line_num(source_code, line_num))
    ...
    prefix_block_code = sitter.get_text(prefix_node)
    suffix_block_code = sitter.get_text(suffix_node)
...
def vue_to_html_ts(prefix, suffix):
    ...
    """
    language = FrontLanguageEnum.VUE.value
    if (f"\n{VueTagConst.TS_START}" in prefix or prefix.startswith(VueTagConst.TS_START)) and (
        ...
        language = FrontLanguageEnum.TS.value
...
def completion_make_cache(cache, completion_cache_time, prompt, text):
    ...
    cache.set(wrap_completion_key(hash_key), json.dumps(text))
    cache.expire(wrap_completion_key(hash_key), completion_cache_time)
...
def get_completion_cache(cache, completion_cache_time, prompt):
    if not cache.enabled:
        return []
    hash_key = get_completion_cache_key(prompt)
    text = cache.get(wrap_completion_key(hash_key))
...
def wrap_completion_key(key):
    return f"completion_cache:{key}"
...
def cut_suffix_overlap(text, prefix, suffix, cut_line=3, ignore_overlap_len=8):
    """
    去除「补全内容」与suffix的前缀重叠部分
    :param text: 补全内容
    :param prefix: 上文
    :param suffix: 下文
    :param cut_line: 截取suffix的行数
    :param ignore_overlap_len: 忽略重叠内容长度
    :return:
    """
    if not text:
        return text
    ...
    for _ in range(cut_line):
        ...
        for i in range(max_overlap_length, int(max_overlap_length / 2), -1):
            ...
            # 一旦text 和 suffix 存在重叠部分，立刻返回非重叠部分
            if text[text_len - i:] == suffix[:i]:
                # 避免误判切割到有效结束符，因此当需切割的内容长度小于等于某个值时，不进行重叠内容切割
                if len(suffix[:i].strip()) > ignore_overlap_len:
                    return text[:text_len - i]
        suffix = "\n".join(spilt_suffix[1:])
    return text


def cut_prefix_overlap(text, prefix, suffix, cut_line=3):
    """
    去除「补全内容」与prefix的后缀重叠部分
    当补全内容行数小于3时，完全匹配前缀，则直接返回空补全
    当补全内容行数大于等于3时，若满足以下条件，则直接返回空补全
        - 首部连续重叠行大于等于3
        - 总重叠行数占比大于60%
    :param text: 补全内容
    :param prefix: 上文
    :param suffix: 下文
    :param cut_line: 截取prefix的行数
    :return:
    """
    ...
    if len(split_text) < 3:
        if judge_prefix_full_line_repetitive(text, prefix):
            return ""
        return text
    ...
    match_line = min(len(split_prefix), len(split_text))
    pattern_text_list = split_text[:match_line]

    for i in range(cut_line):
        cur_match_text_list = split_prefix[len(split_prefix) - match_line - i:len(split_prefix) - i]

        match_count = 0
        continue_flag = True
        for j in range(len(cur_match_text_list)):
            if cur_match_text_list[j].strip() == pattern_text_list[j].strip():
                match_count += 1
                if match_count == 3 and continue_flag:
                    return ""
                if match_count / match_line >= 0.6:
                    return ""
            else:
                continue_flag = False
    return text
...
def judge_prefix_full_line_repetitive(completion_text, prefix):
    """
    判断补全内容是否为与前缀内容尾行完全重复
    :param completion_text: 补全内容
    :param prefix: 待匹配内容
    :return:
    """
    if not prefix or not completion_text:
        return False

    split_prefix_text = prefix.split("\n")
    # 若将同行光标前的内容拼接到补全内容中，便于完全匹配
    cursor_line_prefix_text = split_prefix_text[-1]
    completion_text = cursor_line_prefix_text + completion_text

    split_completion_text = completion_text.split("\n")
    split_completion_text = [i for i in split_completion_text if i.strip()]
    split_prefix_text = split_prefix_text[:-1]
    split_prefix_text = [i for i in split_prefix_text if i.strip()]

    # 若补全内容行数大于匹配内容行数，则直接返回False
    if len(split_completion_text) > len(split_prefix_text):
        return False

    split_prefix_text = split_prefix_text[-len(split_completion_text):]
    for i in range(len(split_completion_text)):
        if split_completion_text[i] != split_prefix_text[i]:
            return False
    return True
...
def is_extreme_repetition(code):
    """
    「原理描述」：
        - 无极端重复内容出现时，行数通常都很长，设置行数阈值小于5不进行该算法判定。
        - 若最长公共子序列（LCS）的长度大于5且不小于行数的一半，则进行匹配过程。
        - 匹配过程中，若LCS重复的次数超过补全总行数的70%或超过8行，则认为存在极端重复。

    「时间复杂度分析」：
        - 假设总字符数为m，列数为n，本算法最坏情况下的时间复杂度为O(n * m)。
        - 根据补全业务的特点，m通常不超过30，n通常不超过1500，因此数据规模不高于在5*10^4。
        - 一般而言，计算机每秒可处理的数据规模远超5*10^8。因此，在补全业务的极端情况下，本算法的计算时间预计不会超过100毫秒。
    ...
    if len(lines) < 5:
        return False, None, None
    n = len(lines)
    for i in range(n - 1):  # 小于 O(n * m)
        lcs = longest_common_substring(lines[i], lines[i + 1])  # 小于 O(m)
        first_line_lcs_index = lines[i].index(lcs)
        if len(lcs) > 5 and len(lcs) >= len(lines[i]) // 2:
            count = sum(1 for k in range(i + 1, n)
                        if lcs in lines[k] and lines[k].index(lcs) == first_line_lcs_index)  # 小于 O(m)
            if count > 8 or count > n // 2:
