...
ENABLE_CONTEXT_DEF_SEARCH = None
ENABLE_CONTEXT_SEMANTIC = None


def init_context_config():
    global ENABLE_CONTEXT_DEF_SEARCH, ENABLE_CONTEXT_SEMANTIC, CONTEXT_COST_TIME

    ENABLE_CONTEXT_DEF_SEARCH = os.environ.get("DISABLE_CONTEXT_DEF_SEARCH", "false").lower() == "false"
    ENABLE_CONTEXT_SEMANTIC = os.environ.get("DISABLE_CONTEXT_SEMANTIC", "false").lower() == "false"
...
async def request_context(client_id: str, codebase_path: str, file_path: str,
    ...
    async with (aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False, limit_per_host=10, keepalive_timeout=3))
                as session):
        tasks = []
        global ENABLE_CONTEXT_SEMANTIC, ENABLE_CONTEXT_DEF_SEARCH
        # 定义检索
        if code_snippets and ENABLE_CONTEXT_DEF_SEARCH:
            for code_snippet in code_snippets:
                if code_snippet:
                    tasks.append(
                        search_definition(session, client_id, codebase_path, file_path, code_snippet, None, None, headers=headers))
        definition_index = len(tasks)

        # 语义检索
        if query and ENABLE_CONTEXT_SEMANTIC:
            for q in query:
                if q is not None:
                    tasks.append(search_semantic(session, client_id, codebase_path, q, headers=headers))
        ...
        try:
            ...
            # 执行所有任务，并设置超时
            done, pending = await asyncio.wait([task for task, _ in indexed_tasks], timeout=CONTEXT_COST_TIME)
...
def get_context(client_id: str, project_path: str, file_path, prefix: str, suffix: str,
                import_content: str | None = None, headers: dict = None) -> str:
    """获取上下文信息
    Args:
        client_id (str): 客户端id
    ...
    definition, semantic = asyncio.run(request_context(client_id,
                                                       ...
                                                       file_path,
                                                       definition_code_snap,
                                                       [semantic_search_content], headers=headers))
    # 解析返回的语义检索结果,返回全部结果
